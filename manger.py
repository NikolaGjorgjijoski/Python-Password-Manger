...
def create_password(pass_key, service, admin_pass):
    return sha512(admin_pass.encode('utf-8') + service.lower().encode('utf-8') + pass_key.encode('utf-8')).hexdigest()[:72]

def get_hex_key(admin_pass, service):
    return sha512(admin_pass.encode('utf-8') + service.lower().encode('utf-8')).hexdigest()
